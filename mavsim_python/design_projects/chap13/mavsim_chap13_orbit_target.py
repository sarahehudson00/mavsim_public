...
mav_python_dir = os.fspath(Path(__file__).parents[2])
# sys.path.insert(0, mav_python_dir)
sys.path.append(mav_python_dir)
...
while sim_time < SIM.end_time:
    ...
    gimbal_cmd = gimbal.pointAtPosition(
        estimated_state, target.position()
    ...
    if ANIMATION:
        world_view.update(
            mav.true_state, target.position(), path, waypoints, world_map
        )  # plot world
    if DATA_PLOTS:
        plot_time = sim_time
        data_view.update(
