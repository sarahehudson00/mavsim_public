...
class PlannerViewer:
    def __init__(self, app):
        ...
        self.window.setCameraPosition(
            pos=center, distance=scale, elevation=50, azimuth=-90
        )
        self.window.setBackgroundColor("k")  # set background color to black
        self.window.show()  # display configured window
        self.blue = np.array([[30, 144, 255, 255]]) / 255.0
        self.red = np.array([[204, 0, 0]]) / 255.0
        self.green = np.array([[0, 153, 51]]) / 255.0

    def draw_tree_and_map(
        self, world_map, tree, waypoints, smoothed_waypoints, radius, dubins_path=None
    ):
        self.window.clear()
        ...
        for i in range(1, tree.num_waypoints):
            ...
            tree_plot_object = gl.GLLinePlotItem(
                pos=points, color=tree_color, width=2, antialias=True, mode="line_strip"
            )
    ...
    def get_straight_line_points(self, tree, i):
        ...
        points = np.concatenate(
            (self.column(points, i).T, self.column(points, parent).T), axis=0
        )
        return points
    ...
    def get_dubins_points(self, tree, i, radius, dubins_path):
        parent = int(tree.parent.item(i))
        dubins_path.update(
            self.column(tree.ned, parent),
            tree.course[parent],
            self.column(tree.ned, i),
            tree.course[i],
            radius,
        )
